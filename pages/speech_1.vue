<template>
    ...
    <v-row justify="center" align="center" id="embedded_video">
          <v-col cols="12" >

            <div style="width: 640px; height: 360px; position: absolute"></div>
                  ...
                  <span v-for="(sub, key) in item.sub" :key="key">
                    <v-tooltip top>
                      <template v-slot:activator="{ on }">
                        <span v-on="on" v-if="sub.indexOf('_')==-1"> {{ sub }} </span>
                        <span v-on="on" v-if="sub.indexOf('_2')!=-1"> {{ sub.split('_')[0] }} </span>
                        <span v-on="on" v-if="sub.indexOf('_3')!=-1"> {{ sub.split('_')[0] }} </span>
                        <span v-on="on" v-if="sub.indexOf('_4')!=-1"> {{ sub.split('_')[0] }} </span>
                        <span v-on="on" v-if="sub.indexOf('_5')!=-1"> {{ sub.split('_')[0] }} </span>
                        <span v-on="on" v-if="sub.indexOf('_6')!=-1"> {{ sub.split('_')[0] }} </span>
                      </template>
                      <span> Tooltip </span>
                    </v-tooltip>
...
export default {
  ...
  methods: {
    ...
    matchHeight() {
      ...
      console.log( 'matchHeight ', heightString + 'px')
